...
원천 CSV/JSON 데이터를 날짜 기준으로 병합하여
`data/daily_merged/{date}_C_raw.csv`,
`data/daily_merged/{date}_C_raw.jsonl`,
`data/daily_merged/{date}_C_label.jsonl` 파일로 저장합니다.
            ...
            for period_dir in sorted(loc_dir.iterdir()):
                ...
                for veh_dir in sorted(period_dir.iterdir()):
                    ...
                    for data_file in sorted(veh_dir.glob('*')):
                        ...
                        # CSV 원천 데이터 병합
                        if suffix == '.csv':
                            csv_out  = out / f"{date}_{location}_raw.csv"
                            jsonl_out = out / f"{date}_{location}_raw.jsonl"
                            ...
                            for chunk in pd.read_csv(data_file, chunksize=chunksize):
                                ...
                                first = False
                            print(f"[MERGE] {data_file} -> {csv_out.name} (+ JSONL)")

                        # JSON 라벨 파일 병합
                        elif suffix == '.json':
                            label_out = out / f"{date}_{location}_label.jsonl"
                            with open(data_file, 'r', encoding='utf-8') as f:
                                ann = json.load(f)
                            with open(label_out, 'a', encoding='utf-8') as fj:
                                fj.write(json.dumps(ann, ensure_ascii=False) + "\n")
                            print(f"[LABEL] {data_file} -> {label_out.name}")


if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='월→날짜→위치→차량ID 구조 V2X 데이터 일별 병합')
